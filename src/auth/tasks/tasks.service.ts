...
export class TasksService {
  ...
  async create(dto: CreateTaskDto, userId: number): Promise<Task> {
    ...
    if (dto.listId != null) {
      const list = await this.listsRepo.findOne({
        where: { id: dto.listId, userId },
      });
      if (!list)
        throw new BadRequestException('List not found or not owned by user');
      listId = list.id;
    }
  ...
  async replace(id: number, dto: PutTaskDto, userId: number): Promise<Task> {
    ...
    existing.dueDate = Object.prototype.hasOwnProperty.call(dto, 'dueDate')
      ? dto.dueDate
        ...
        : null
      : null;
    existing.listId = Object.prototype.hasOwnProperty.call(dto, 'listId')
      ? listId
      : null;
